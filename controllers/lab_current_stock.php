...
class Lab_current_stock extends MY_Controller
{
    private $data;
    ...
    function __construct()
    {
        ...
        $this->load->model('lab_stocks_model');
        $this->load->model('lab_commodity_model');
    ...
    public function show_central_level_stock($period="000000"){
        ...
        $datacl['central_level_data_by_period']=$this->lab_stocks_model->show_current_stock_by_period($period);
        $datacl['central_level'] = $this->lab_stocks_model->show_sorted_central_stock();
        $datacl['commodity'] = $this->lab_commodity_model->show_lab_commodities();
        ...
        $datacl['pending_shipment'] = $this->lab_stocks_model->show_pending_shipments();

        /*show_pending_shipments*/

        $this->load->view('lab_current_stock',$datacl);
    ...
    public function update_central_level(){
        ...
        $data = array(
            'period'=>$today,
            'commodity_id' => $this->lab_commodity_model->get_commodity_id_with_the_given_name($this->input->post('commodity_name')),
        ...
        $this->lab_stocks_model->update_central_data($id,$data);
        $this->show_central_level_stock();
    ...
    public function delete_central_level_data($id){
        ...
        $deleterecord=$this->db->delete('lab_central_level_data');
        $data['status'] =  "";
        if($deleterecord){
    ...
    public function save_central_level(){
        ...
        $values=$this->lab_stocks_model->show_pending_shipment_by_id($pid);
        ...
        $this->lab_stocks_model->add_central_stock($data_array);
        $fetched_data=$this->lab_stocks_model->show_pending_shipment($pid);
       ...
       $this->lab_stocks_model->update_pending_shipment($pid, $second_array);
       $this->show_central_level_stock();

        }else{
            $this->lab_stocks_model->delete_pending_data($pid);
            $this->show_central_level_stock();
        }
    }else{
    ...
    function get_by_id()
    {
        ...
        $data=$this->lab_stocks_model->show_pending_shipment_by_id($pending_shipment_id);
        
        $data_array = array();
        foreach($data as $myvalue){
        $commodity_id=$myvalue->commodity_id;
        $funding_agency_id=$myvalue->funding_agency_id;
        $funding_agency_name=$this->lab_stocks_model->get_funding_agency_name($funding_agency_id);
        $commodity_name= $this->lab_stocks_model->get_commodity_id_with_the_given_id($commodity_id);
