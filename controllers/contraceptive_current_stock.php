...
class Contraceptive_current_stock extends MY_Controller
{
    private $data;
    ...
    function __construct()
    {
        ...
        $this->load->model('fp_stocks_model');
        $this->load->model('fp_commodity_model');
    ...
    public function show_central_level_stock($period="000000"){
        ...
        $datacl['central_level_data_by_period']=$this->fp_stocks_model->show_current_stock_by_period($period);
        $datacl['central_level'] = $this->fp_stocks_model->show_sorted_central_stock();
        $datacl['commodity'] = $this->fp_commodity_model->show_contraceptive_commodities();
        ...
        $datacl['pending_shipment'] = $this->fp_stocks_model->show_pending_shipments();

        /*show_pending_shipments*/

        $this->load->view('fp_current_stock',$datacl);
    ...
    public function update_central_level(){
        ...
        $data = array(
            'period'=>$today,
            'commodity_id' => $this->fp_commodity_model->get_commodity_id_with_the_given_name($this->input->post('commodity_name')),
        ...
        $this->fp_stocks_model->update_central_data($id,$data);
        $this->show_central_level_stock();
    ...
    public function delete_central_level_data($id){
        ...
        $deleterecord=$this->db->delete('contraceptive_central_level_data');
        $data['status'] =  "";
        if($deleterecord){
    ...
    public function save_central_level(){
        ...
        $values=$this->fp_stocks_model->show_pending_shipment_by_id($pid);
        ...
        $this->fp_stocks_model->add_central_stock($data_array);
        $fetched_data=$this->fp_stocks_model->show_pending_shipment($pid);
       ...
       $this->fp_stocks_model->update_pending_shipment($pid, $second_array);
       $this->show_central_level_stock();

        }else{
            $this->fp_stocks_model->delete_pending_data($pid);
            $this->show_central_level_stock();
        }
    }else{
    ...
    function get_by_id()
    {
        ...
        $data=$this->fp_stocks_model->show_pending_shipment_by_id($pending_shipment_id);
        
        $data_array = array();
        foreach($data as $myvalue){
        $commodity_id=$myvalue->commodity_id;
        $funding_agency_id=$myvalue->funding_agency_id;
        $funding_agency_name=$this->fp_stocks_model->get_funding_agency_name($funding_agency_id);
        $commodity_name= $this->fp_stocks_model->get_commodity_id_with_the_given_id($commodity_id);
