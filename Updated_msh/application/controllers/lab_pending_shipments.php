...
class Lab_pending_shipments extends MY_Controller
{
    private $data;
    ...
    {
        parent::__construct();
        $this->load->model('lab_pending_shipments_model');
    }
    ...
    public function show_pending_shipments() {
        ...
        {
            $data2['PSTOCKS'] = $this->lab_pending_shipments_model->show_pending_shipment($period_posted);
            $data2['pending_shipment_successfully_retrieved'] = "retrieved";// display in view only if data is retrieved

        }
        $data2['COMMODITY']=$this->lab_pending_shipments_model->show_commodities();
        $data2['FUNDING']=$this->lab_pending_shipments_model->show_fundingorgs();
        $data2['pending_shipment_successfully_retrieved'] = "";
        $data2['ALL_SHIPMENTS'] = $this->lab_pending_shipments_model->show_all_pending_shipment();


        $this->load->view('lab_pending_shipments',$data2);
    }
    public function save_pending_shipment(){
        ...
        $commodity_id=$this->lab_pending_shipments_model->get_commodity_id_with_the_given_name($commodity_name);
        $funding_agency_Id=$this->lab_pending_shipments_model->get_funding_agency_id($funding_agency);
        ...
        $psId = $this->lab_pending_shipments_model->add_pending_shipment($pendingStock);
        $data['pending_delivery_message'] =  "";
        if($psId){
            $data['pending_delivery_message'] =  "The Pending Stock was saved successfully!..";
        }

        $query = $this->lab_pending_shipments_model->show_all_pending_shipment();
        if($query){
    ...
    public function update_pending_shipment_id() {
        ...
        $commodity_id=$this->lab_pending_shipments_model->get_commodity_id_with_the_given_name($commodity_name);
        $funding_agency_id=$this->lab_pending_shipments_model->get_funding_agency_id($funding_agency_name);
        $data = array(
        ...
        $this->lab_pending_shipments_model->update_pending_shipment($id,$data);
