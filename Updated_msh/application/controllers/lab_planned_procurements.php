...
class Lab_Planned_procurements extends MY_Controller
{
    private $data;
    ...
    {
        parent::__construct();
        $this->load->model('lab_planned_procurements_model');
    }
    ...
    public function show_planned_procurements() {
        $data2['ALL_SHIPMENTS'] = $this->lab_planned_procurements_model->show_all_planned_procurement();
        $data2['COMMODITY']=$this->lab_planned_procurements_model->show_commodities();
        $data2['FUNDING']=$this->lab_planned_procurements_model->show_fundingorgs();
        $this->load->view('lab_planned_procurements',$data2);
    }
    public function show_planned_procurements_from_selected_planned_delivery_date() {
        ...
        $data2['PSTOCKS'] = $this->lab_planned_procurements_model->show_planned_procurement($planned_delivery_date);
        $data2['single_PSTOCKS'] = $this->lab_planned_procurements_model->show_planned_procurement_id($psid);
        $data2['COMMODITY']=$this->lab_planned_procurements_model->show_commodities();
        $data2['FUNDING']=$this->lab_planned_procurements_model->show_fundingorgs();
        $data2['planned_procurement_successfully_retrieved'] = "";
        $data2['ALL_SHIPMENTS'] = $this->lab_planned_procurements_model->show_all_planned_procurement();
        if ($data2['PSTOCKS'] = $this->lab_planned_procurements_model->show_planned_procurement($planned_delivery_date)){
    ...
    public function save_planned_procurement(){
        ...
        $commodity_id=$this->lab_planned_procurements_model->get_commodity_id_with_the_given_name($commodity_name);
        $funding_agency_id=$this->lab_planned_procurements_model->get_funding_agency_id($funding_agency);
        ...
        $psId = $this->lab_planned_procurements_model->add_planned_procurement($pendingStock);
        $data['pending_delivery_message'] =  "";
        if($psId){
            $data['pending_delivery_message'] =  "The Pending Stock was saved successfully!..";
        }

        $query = $this->lab_planned_procurements_model->show_all_planned_procurement();
        if($query){
    ...
    public function update_planned_procurement_id() {
        ...
        $commodity_id=$this->lab_planned_procurements_model->get_commodity_id_with_the_given_name($commodity_name);
        $funding_agency_id=$this->lab_planned_procurements_model->get_funding_agency_id($funding_agency_name);
        $data = array(
            'planned_delivery_date'=>$this->input->post('planned_delivery_date'),
            'unit_of_measure'=>$this->input->post('unit_of_measure'),
            'quantity' => $this->input->post('quantity'),
        ...
        $this->lab_planned_procurements_model->update_planned_procurement($id,$data);
